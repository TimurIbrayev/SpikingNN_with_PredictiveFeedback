...
from self_models.vgg_wFeedback_spiking_target_objective_1 import VGG_SNN_STDB_wFeedback
...
identifier = 'snn_'+architecture.lower()+'_'+dataset.lower()+'_'+str(timesteps)+'_tradeoff'+str(trade_off)+'_type'+trade_off_type.lower()+'_posthoc_analysis_epsilon_'+str(epsilon)
...
with torch.no_grad():
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        output, predictions, errors, spike_count         = model(data)
        loss_cls                                         = F.cross_entropy(output,target)
        # Reconstruction loss
        if trade_off_type == 'last_t':
            loss_reconstruction             = errors[-1].sum()/data.size(0)
        ...
        losses_total.update(loss.item(), data.size(0))
        top1.update(correct.item()/data.size(0), data.size(0))
        spikes.update(spike_count.item()/data.size(0), data.size(0))
...
f.write('\n Average number of spikes: {:.4f}'.format(spikes.avg))
