...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 20 15:36:03 2021

@source: Nitin Rathi

@author: tibrayev

This is a script to train ann with feedback only.
For training ann without feedback use ann.py.
This script is extended and based on ann.py written by Nitin Rathi
...
import os
import numpy as np
from self_models.vgg_wFeedback import VGG_wFeedback as VGG
...
def train(epoch, loader):
    ...
    for batch_idx, (data, target) in enumerate(loader):
        ...
        loss_reconstruction                 = sum(errors).sum()/data.size(0)
    ...
    f.write('\n Epoch: {}, lr: {:.1e}, train_loss: {:.4f} | {:.4f} | {:.4f}, train_acc: {:.4f}'.format(
            epoch,
            learning_rate,
            losses_cls.avg,
            losses_recon.avg,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if batch_idx == 0:
                ...
                filename = './trained_models/ann/'+identifier+'.png'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trade_off',              default=10.0,               type=float,     help='trade-off between classification and reconstruction losses')
    ...
    identifier = 'ann_'+architecture.lower()+'_t'+str(timesteps)+'_'+dataset.lower()+'_tradeoff'+str(trade_off)+'_lr'+str(learning_rate)
    ...
    if architecture[0:3].lower() == 'vgg':
        model = VGG(vgg_name=architecture, timesteps=timesteps, labels=labels, dataset=dataset, kernel_size=kernel_size, dropout=dropout)
