...
from self_models.vgg_wFeedback_spiking import VGG_SNN_STDB_wFeedback
...
identifier = 'snn_'+architecture.lower()+'_'+dataset.lower()+'_'+str(timesteps)+'_tradeoff'+str(trade_off)+'_type'+trade_off_type.lower()+'_posthoc_analysis'
...
for t in range(len(predictions)):
    imgs = ((predictions[t] - 0.5)/(1.0-0.5))*(1.0-0.0) - 0.0
    diff = np.logical_not(np.logical_and((imgs < data+epsilon).cpu(),(imgs > data-epsilon).cpu()))*1.0
    ...
    differences_over_t.append(vutils.make_grid(diff.cpu(), padding=2))

plt.figure(), plt.imshow(data[0,0].cpu())
plt.figure(), plt.imshow(imgs[0,0].cpu())
plt.figure(), plt.imshow(diff[0,0].cpu())
...
for i in range(len(predictions_over_t)):
    ...
    ims.append([ax1.imshow(np.transpose(predictions_over_t[i],(1,2,0)), animated=True),
                ax2.imshow(np.transpose(differences_over_t[i],(1,2,0)), animated=True),
                frm])
...
# saving as .git format:
writergif = animation.PillowWriter(fps=2, metadata=dict(artist='TimurIbrayev'))
ani.save('./trained_models/snn/'+identifier+'_predictions_and_binarydiff_epsilon={}.gif'.format(epsilon),
         writer=writergif)

# # saving as .mp4 format:
# writervideo = animation.FFMpegWriter(fps=2, metadata=dict(artist='TimurIbrayev'))
# ani.save('./trained_models/snn/'+identifier+'_predictions_and_binarydiff_epsilon={}.mp4'.format(epsilon),
#          writer=writervideo)
