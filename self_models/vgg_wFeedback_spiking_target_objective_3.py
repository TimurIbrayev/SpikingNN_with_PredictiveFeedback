...
class VGG_SNN_STDB_wFeedback(nn.Module):
    ...
    def forward(self, x, find_max_mem=False, max_mem_layer=0):
        ...
        for t in range(self.timesteps):
            input_x_normalized = self.zero_mean_unit_var_and_shiftminmax(input_x)
            out_prev = self.input_layer(input_x_normalized)
            ...
            if not find_max_mem:
                ...
                outputs.append(self.mem[prev+l+1])
        
        if find_max_mem:
            return max_mem
        else:
            return self.mem[prev+l+1], predicts, errors, outputs
